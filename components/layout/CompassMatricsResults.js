      ...
      >
          ...
          <Col lg="12" md="12" sm="12" xs="12" className="py-5">
            {s ? (
              <h3 className={styles.srchresultstitle}>
                {" "}
                Trends related to :{" "}
                <span className={styles.srchkeyword}>
                  {decodeURI(s.charAt(0).toUpperCase() + s.slice(1))}
                </span>
              </h3>
            ) : (
        ...
        <Row className={`${isgridActive} mb-3`}>
          ...
          {apiHit && data.length == 0 && (
            <Col>
              <Alert color="primary">Opps ! No Result Found ..</Alert>
            </Col>
          )}
        </Row>
      </Container>
...
export default CompassMatricsResults;
